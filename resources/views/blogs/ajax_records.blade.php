    ...
    <table class="table">
        ...
        <tbody>
            @if (isset($data['records']) && count($data['records'])>0)
                ...
                @php $any_blog_found = false; @endphp
                        ...
                        <td>
                            <div class="display_images_list">
                                <span class="avatar-color">
                                    <a data-fancybox="demo" data-src="{{ is_image_exist($item['blog_image']) }}">
                                        <img title="{{ $item['blog_title'] }}" src="{{ is_image_exist($item['blog_image']) }}" height="100">
                                    </a>
                                </span>                            
                            </div>
                        </td>
                        <td>{{ $item['blog_title'] }}</td>
                        <td><span class="role-badge" style="background-color: {{ $item['blog_status'] == 'Published'? '#455356':'#b4b5af' }}">{{ $item['blog_status'] }}</span></td>
                        <td>
                            ...
                            <div class="dropdown">
                                ...
                                @php $any_blog_found = true; @endphp
                                <div class="dropdown-menu">
                                    @can('blog-edit')
                                    <a class="dropdown-item" href="{{ url('blog')}}/{{$item['id']}}/edit" >
